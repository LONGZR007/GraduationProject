...
int8_t RTC_Configuration(void)
{
	...
	RCC_RTCCLKConfig(RCC_RTCCLKSource_HSE_Div128);//RCC_RTCCLKSource_LSE
...
//	RTC_WaitForLastTask();
	...
	RTC_SetPrescaler(62500-1); /* RTC period = RTCCLK/RTC_PR = (32.768 KHz)/(32767+1) = 1HZ */
...
//	RTC_WaitForLastTask();
    ...
    DecTm->tm_wday  = SrcTm->tm_wday;
 
    /* 时间格式转换 */
    SrcTm->tm_year =SrcTm->tm_year - 2000;
    
//    printf("\r\n %d,%d,%d  %d:%d:%d",SrcTm->tm_year,SrcTm->tm_mon,SrcTm->tm_mday,SrcTm->tm_hour,SrcTm->tm_min,SrcTm->tm_sec);
    Metric_BCD_convert(SrcTm);
    
    /* 设置时间 */
    DS_DateTime_Set(SrcTm);   // 设置时间
