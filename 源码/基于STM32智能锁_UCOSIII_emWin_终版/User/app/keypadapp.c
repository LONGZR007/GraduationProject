...
*********************************************************************************************************
*/
static char InputBuff[7];
static u8 XC;
void __EDIT_AddKey(EDIT_Handle Edit, char s_Textbuf);
						...
						{		
							...
							if(s_Key == '1')
							{
								...
								else
								{
									...
									{								
										EDIT_GetText(hEdit,s_Editbuf,20);
							...
							{			
								...
								if(s_Offset == 0)
								{
									...
									{
										if(editflag)
                                            MULTIEDIT_AddKey(hEdit, GUI_KEY_BACKSPACE);
										else 
                                        {
//                                            if (XC > 0)
//                                            {
...
//                                EDIT_SetText(hEdit, InputBuff);
//                            }
							if(editflag)
                                MULTIEDIT_AddText(hEdit, s_Textbuf);
							else 
                                EDIT_AddKey(hEdit,s_Textbuf[0]);
						}
					}
				}
				break;
...
{
//	WM_DeleteWindow(KEYBOARD);    // 删除键盘窗口
//	keypadflag = 0;
}
...
void __EDIT_AddKey(EDIT_Handle Edit, char s_Textbuf)
{
    ...
    *uBuff = s_Textbuf;
    ...
    EDIT_GetText(Edit, Buff, 7);   // 获取字符串
    
//    if ()
        strncat(Buff, uBuff, 1);
        EDIT_SetFont(Edit, GUI_FONT_16B_ASCII);
