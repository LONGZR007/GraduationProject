...
Reg_Info Sensor_Config[] =
{
	...
	{COM3,		  0x10},/*Horizontal mirror image*/
...
/**
  ...
  * @retval 无
  */
static void OV7725_Window_Set(uint16_t sx,uint16_t sy,uint16_t width,uint16_t height,uint8_t QVGA_VGA)
{
	...
	{
		/*QVGA RGB565 */
		SCCB_WriteByte(0x12,0x46); // REG_COM7
	}
	else
	{
			/*VGA RGB565 */
	...
	SCCB_ReadByte(&reg_raw,1,0x17); // REG_HSTART
	
	//sx为窗口偏移，高8位存储在HSTART，低2位在HREF
	cal_temp = (reg_raw + (sx>>2));	
	SCCB_WriteByte(0x17,cal_temp ); // REG_HSTART
	
	/***************HSIZE*********************/
	//水平宽度，高8位存储在HSIZE，低2位存储在HREF
	SCCB_WriteByte(0x18,width>>2);//HSIZE左移两位 //REG_HSIZE
	
	
	/***************VSTART*********************/
	//读取寄存器的原内容，VStart包含偏移值，在原始偏移植的基础上加上窗口偏移	
	SCCB_ReadByte(&reg_raw,1,0x19);	//REG_VSTRT
	//sy为窗口偏移，高8位存储在HSTART，低1位在HREF
	cal_temp = (reg_raw + (sy>>1));	
	
	SCCB_WriteByte(0x19,cal_temp);//REG_VSTRT
	
	/***************VSIZE*********************/
	//垂直高度，高8位存储在VSIZE，低1位存储在HREF
	SCCB_WriteByte(0x1A,height>>1);//VSIZE左移一位//REG_VSIZE
	
	/***************VSTART*********************/
	//读取寄存器的原内容	
	SCCB_ReadByte(&reg_raw,1,0x32);	//REG_HREF
	//把水平宽度的低2位、垂直高度的低1位，水平偏移的低2位，垂直偏移的低1位的配置添加到HREF
	cal_temp = (reg_raw |(width&0x03)|((height&0x01)<<2)|((sx&0x03)<<4)|((sy&0x01)<<6));	
	
	SCCB_WriteByte(0x32,cal_temp);//REG_HREF
	
	/***************HOUTSIZIE /VOUTSIZE*********************/
	SCCB_WriteByte(0x29,width>>2);//REG_HOutSize
	SCCB_WriteByte(0x2C,height>>1);//REG_VOutSize
	
	//读取寄存器的原内容	
	SCCB_ReadByte(&reg_raw,1,0x2A);	//REG_EXHCH
	cal_temp = (reg_raw |(width&0x03)|((height&0x01)<<2));	

	SCCB_WriteByte(0X2A,cal_temp);	//REG_EXHCH
}
...
ErrorStatus Ov7725_Init(void)
{
	...
	OV7725_Window_Set(200, 80, 240, 320, 1);// 设置为竖屏模式
