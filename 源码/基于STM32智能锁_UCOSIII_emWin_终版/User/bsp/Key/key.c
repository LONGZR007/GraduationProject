...
static void NVIC_Configuration(void)
{
  ...
  /* 配置中断源：按键1 */
  NVIC_InitStructure.NVIC_IRQChannel = Bell_OUT_INT_EXTI_IRQ;
  /* 配置抢占优先级 */
  NVIC_InitStructure.NVIC_IRQChannelPreemptionPriority = 1;
  /* 配置子优先级 */
  NVIC_InitStructure.NVIC_IRQChannelSubPriority = 1;
...
void EXTI_Key_Config(void)
{
	...
	RCC_APB2PeriphClockCmd(Bell_OUT_INT_GPIO_CLK,ENABLE);
...
/*--------------------------门内门铃配置-----------------------------*/
  ...
  /*--------------------------门外门铃配置-----------------------------*/
	/* 选择按键用到的GPIO */	
  GPIO_InitStructure.GPIO_Pin = Bell_OUT_INT_GPIO_PIN;
  /* 配置为浮空输入 */	
  GPIO_InitStructure.GPIO_Mode = GPIO_Mode_IPU;
  GPIO_Init(Bell_OUT_INT_GPIO_PORT, &GPIO_InitStructure);

	/* 选择EXTI的信号源 */
  GPIO_EXTILineConfig(Bell_OUT_INT_EXTI_PORTSOURCE, Bell_OUT_INT_EXTI_PINSOURCE); 
  EXTI_InitStructure.EXTI_Line = Bell_OUT_INT_EXTI_LINE;
