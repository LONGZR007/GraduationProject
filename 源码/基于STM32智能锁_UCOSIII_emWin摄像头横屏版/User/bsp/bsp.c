...
FATFS fs_flash;	/* Work area (file system object) for logical drives */
FATFS fs_sd;
...
void BSP_Init(void)
{  
#if  SET_Password    /* 设置管理员密码用 */
	u8 EnterPassword[] = {'1','2','3','4','5','6','\0',};
#endif
...
//    cm_backtrace_init("Template", HARDWARE_VERSION, SOFTWARE_VERSION);
	...
	result=f_mount(&fs_sd,"1:",1);
    result=f_mount(&fs_flash,"0:",1);
	//printf("mount_res:%d\n",result);
	if(result!=FR_OK)
	{
			IsCal=0xE0;
...
//		printf("YPos1:%2d-->%4d && YPos2:%3d-->%4d\n",readCal[1], readCal[7], readCal[5], readCal[3]);
	...
	SPI_FLASH_BufferWrite((u8 *)EnterPassword, AdministratorPassword_Addr, 7);   // 写数据
#endif
