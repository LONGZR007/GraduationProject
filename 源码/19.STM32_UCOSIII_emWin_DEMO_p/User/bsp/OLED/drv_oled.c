...
void OLED_Init_Delay(void)
{
    u32 xC;
    for(xC=0; xC<0xFFFFF; xC++);
}
...
{      	
    ...
    SPI_FLASH_BufferRead((u8 *)Buff, 2050*4096+c*16, 16);

    OLED_Set_Pos(x,y);	
    
    for(i=0;i<8;i++)
        OLED_WR_Byte(Buff[i],OLED_DATA,aga);
    
    OLED_Set_Pos(x,y+1);
    
    for(i=0;i<8;i++)
        OLED_WR_Byte(Buff[i+8],OLED_DATA,aga);
